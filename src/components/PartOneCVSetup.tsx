...
export const PartOneCVSetup: React.FC<PartOneCVSetupProps> = ({
  ...
  const [directConvertedNotice, setDirectConvertedNotice] = useState<string | null>(null);
    ...
    <div className="space-y-6">
            ...
            <div>
              ...
              {uploadSuccess && (
                <div className="mt-3 space-y-2.5">
                  ...
                  {/* Post-Upload Decision Bar: Convert Directly vs Match Job */}
                  <div className="p-3 bg-gradient-to-r from-red-50/80 to-slate-50 border border-red-200 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[11px] text-slate-600">
                        Convert directly to ATS single-column format or match with a job posting.
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      <button
                        id="btn-direct-ats-convert-upload"
                        onClick={handleQuickConvert}
                        className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-md flex items-center gap-1.5 transition-colors shadow-2xs"
                      >
                        <Zap className="w-3 h-3 text-amber-300 fill-amber-300" />
                        <span>⚡ Convert to ATS & Export</span>
                      </button>
                      <button
                        id="btn-continue-step2-upload"
                        onClick={onNext}
                        className="px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-md flex items-center gap-1 transition-colors"
                      >
                        <span>Match Job</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
        ...
        {showEditor ? (
          ...
          <div className="mt-4 space-y-3">
            {/* Direct ATS Converter Feature Card */}
            <div className="p-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-xl shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                    ...
                    Convert Existing CV to ATS-Friendly (No Job Matching Needed)
                  </h4>
                </div>
                <p className="text-xs text-slate-300 max-w-xl">
                  Already have a CV that isn't passing ATS scans? Convert it immediately into a clean, single-column, machine-readable format for Workday, Taleo, Greenhouse, and Lever.
                </p>
              </div>

              <button
                id="btn-direct-ats-convert-banner"
                onClick={handleQuickConvert}
                className="px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg flex items-center gap-2 transition-all shadow-xs shrink-0 self-stretch md:self-auto justify-center"
              >
                <Zap className="w-4 h-4 text-amber-300 fill-amber-300" />
                <span>⚡ Convert to ATS Format & Export</span>
              </button>
            </div>

            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="text-xs text-slate-600">
                <span className="font-semibold text-slate-800">Custom edits?</span> You can tweak individual text fields or bullet points anytime.
              </div>
              <button
                onClick={() => setShowEditor(true)}
                className="text-xs font-semibold text-blue-600 hover:text-blue-800 underline transition-colors"
              >
                Open manual field editor
              </button>
            </div>
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
        <div className="flex items-center gap-2 text-xs text-slate-600 font-medium">
          ...
          <span>Step 1 Complete: Candidate profile ready.</span>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full sm:w-auto">
          <button
            id="btn-direct-ats-convert-bottom"
            onClick={handleQuickConvert}
            ...
            title="Skip job description and convert directly into standard ATS single-column format"
          >
            <Zap className="w-3.5 h-3.5 text-amber-300 fill-amber-300" />
            <span>⚡ Just Convert to ATS & Export</span>
          </button>
          ...
          >
            <span>Continue to Step 2: Target Job (Optional)</span>
